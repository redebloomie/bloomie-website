...
if (isset($_GET['id'])) {
  ...
  $detalhesQuery = "SELECT p.*, u.foto_perfil, u.usuario 
                    FROM post p
...
<html lang="en">
  ...
  <body id="homepage">
    ...
    <main>
      ...
      >
        <p class="h3 txtT">Banimento do post</p>
        <div class="border border-primary rounded-4 col-12 col-md-12 p-3">
          ...
          <div class="d-flex align-items-center col-12 col-md-12 mb-4">
              <div class="d-flex linha col-12">
                <p class="txtj mb-0" style=" word-wrap: break-word; max-width: 100%;">"<?php echo $detalhes['texto'] ?>"</p>
              </div>
            </div>
            <?php
              ...
              if (!empty($detalhes['imagem'])) {
                  echo '
                      <div class="d-flex align-items-center col-12 col-md-12 mb-4">
                          <div class="d-flex linha col-12">
                              <img class="txtj mb-0" style="height:20vw; width: 30vw; object-fit: cover; border-radius: 20px" src="' . $detalhes['imagem'] . '">
                          </div>
                      </div>
                  ';
              }
            ?>
          <p class="txtj">Postado em:</p>
          <div class="col-12 col-md-6 col-lg-4 mb-4">
            <p
              class="form-control rounded-4"
              style="border-color: #1185e3"><?php echo $detalhes['data_publicacao'] ?></p>
          </div>
          ...
          <div class="d-flex justify-content-center align-items-center">
            ...
            <button
              ...
              style="height: 7vh" onclick="atualizarStatus(<?php echo $detalhes['ID_post'] ?>)">
              <span>Confirmar</span>
            </button>
          </div>
        </div>
    ...
    <script>
      function atualizarStatus(ID_post) {
        ...
        xhr.open('POST', 'adm_banirPost.php', true);
        xhr.setRequestHeader('Content-Type', 'application/x-www-form-urlencoded');
        xhr.onreadystatechange = function () {
          if (xhr.readyState === 4 && xhr.status === 200) {
            var responseData = JSON.parse(xhr.responseText);
            console.log(responseData.response);
            window.location.href = 'adm_postagens.php';
          }
        };

        // Inclua o motivo na requisição
        var dados = 'ID_post=' + ID_post + '&motivo=' + encodeURIComponent(motivo);
        xhr.send(dados);
      }
    </script>
